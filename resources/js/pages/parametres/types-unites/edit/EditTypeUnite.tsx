...
import { NatureResource, TypeOrganisationResource } from "types/organisation.type";
  ...
  return (
    <HookModalForm {...props} modalBodyClassName="bg-light p-3" onClose={props.onExit}>
      <Row className="g-2">
        ...
        <Col sm={3}>
          <TextInput label="Position" name="position" mask={MASK.number} isRequired />
        </Col>
        <Col sm={12}>
 ...
 * @returns
 */
export const EditTypeUnite: FC<EditTypeUniteProps> = ({ closeModal, selected }) => {
  const query = useQueryClient();
  const { data: results } = useQuery({
    queryKey: [QUERY_KEY.natures],
    queryFn: () => natureApi.findAll<NatureResource>().then((e) => e.data.filter((e) => e.code === NATURE.unite)),
  });
