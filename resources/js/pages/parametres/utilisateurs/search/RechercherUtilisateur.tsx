...
import { ListUtilisateur } from "../list/ListUtilisateur";
import { EditUtilisateur } from "../edit/EditUtilisateur";
...
const searchUtilisateurs = ({ queryKey }: any) =>
  userApi.findAll<UserResource>(buildRequestParams(queryKey[1] as RequestParam));
...
const RechercherUtilisateur: FC = () => {
  const search = useSearch();
  const searchParams = parseParams(search.searchParams);
  const [, toggleFilter] = useToggle();

  const query = useQuery({
    queryKey: [QUERY_KEY.utilisateurs, search.queryParams],
    keepPreviousData: true,
    queryFn: searchUtilisateurs,
  });
...
export default RechercherUtilisateur;
