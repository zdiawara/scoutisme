...
const CreatePersonne: FC = () => {
  ...
  useEffect(() => {
    const typePersonneExist = [TYPE_PERSONNES.ADULTE, TYPE_PERSONNES.SCOUT].includes(typePersonne || "");
  ...
  return (
    <PersonneForm
      onSave={create}
      title={`Ajouter ${typePersonne === TYPE_PERSONNES.SCOUT ? " un scout" : " un adulte"}`}
      defaultValues={{
