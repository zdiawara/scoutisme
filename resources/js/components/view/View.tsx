...
const Item: FC<ItemProps> = ({ label, children }) => {
  return (
    <>
      {label && <div className="text-muted text-uppercase fs-6 m-0 mb-1">{label} </div>}
...
const Header: FC<HeaderProps> = ({ label, description, icon, right, className = "shadow-sm p-2" }) => {
  return (
...
const Etat: FC<EtatProps> = ({ value }) => {
  ...
  return <Badge bg={isActive ? "success" : "danger"}>{isActive ? "Actif" : "Inactif"}</Badge>;
};
